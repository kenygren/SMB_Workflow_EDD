...
def update_dwelltime(updated_start, updated_stop, numpts, desired_dwelltime, motor_speed):
    ...
    print("this is the even integer speed:")
    print(even_int_speed)
...
def combine_data_input(f, IntStart, DataNo, Config,  XYZWs , CtTime, bw, bh, rsgap_size,  Scan=0):
    ...
    if Scan == 0:
        ...
        if isinstance(Scan, tuple):
            ...
            if Scan[0] == 2:
                ...
                scan_string = '%d %d %f %f %d %d %f %f %d \n'
                scan_info = Scan
            if Scan[0] == 3:
                # no string in input. for axis0 and axis1: 1=x; 2=y; 3=z; 4=ome
                scan_string = '%d %s %f %f %d %d %f %f %d \n'
                scan_info = Scan
    ...
    for ii in range(XYZWs.shape[0]):
        ...
        scan_info2 = scan_info
        c1 = scan_info2[2]
        c2 = scan_info2[3]
        if ii % 2 == 1:
            scan_info2 = ( scan_info2[0], scan_info2[1],  c2, c1,  scan_info2[4] )
        #print(scan_info2)
